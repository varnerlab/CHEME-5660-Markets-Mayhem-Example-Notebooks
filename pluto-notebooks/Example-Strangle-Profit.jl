...
# ╔═╡ 7061321f-5f03-412b-a4b0-eeca1660ec54
begin
...
# ╔═╡ f92543c1-cd8b-46d6-a2dc-1a05db23000b
md"""
### CHEME 5660: Calculating the Profit of a Strangle at Expiration
"""

# ╔═╡ 7cf8cba1-9064-404d-97bf-d53cf9e1a041
html"""
...
# ╔═╡ 03dfa8a0-fb82-4487-98b3-bf37598765bf
md"""
### Introduction
"""

# ╔═╡ 28306b66-7f20-4f91-8fb9-c0acda729ef4
md"""
### Materials and Methods
"""

# ╔═╡ 06926b6f-05c9-4116-9225-358f8dd2cd60
# ------------------------------------------------------------------------------------------------- #
...
# ╔═╡ 4e1dea00-8f42-404b-97cb-b30aeaf0fe81
md"""
### Results
"""

# ╔═╡ f07d9dab-ea55-4187-a6f9-115a48a6eed7
md"""
#### Case I: Short Strangle
"""

# ╔═╡ 2633b0c4-f95d-4f6d-a3a6-a57c81419da5
let
	...
	# premium data (on 08/05/22 at 11:47 AM ITH)
	Sₒ = 101.57
	𝒫₁ = 4.05 	# Short put at K = 90.0
	𝒫₂ = 5.70  	# Short call at K = 110.0
	
	# build put contract -
	put_contract = PutContractModel()
	put_contract.ticker = T₁
	put_contract.expiration_date = D
	put_contract.strike_price = K₁
	put_contract.premium = 𝒫₁
	put_contract.number_of_contracts = 1
	put_contract.direction = -1 # SHORT
	
	# create a call contract -
	call_contract = CallContractModel()
	call_contract.ticker = T₂
	call_contract.expiration_date = D
	call_contract.strike_price = K₂
	call_contract.premium = 𝒫₂
	call_contract.number_of_contracts = 1
	call_contract.direction = -1 # SHORT

	# build model -
	straddle_contract_array = Array{AbstractAssetModel,1}()
	push!(straddle_contract_array, put_contract)
	push!(straddle_contract_array, call_contract)
	
	# setup the underlying -
	L = 1000
	underlying_range = range(75.0, stop = 125.0, length = L) |> collect
	
	# compute the table -
	dt = expiration(straddle_contract_array, underlying_range)

	# zero line -
	Z = zeros(L)

	# plot -
	plot(dt[!, :S], dt[!,:profit],c=:black, label="total profit", legend=:bottomright, lw=3, 
		bg="floralwhite", background_color_outside="white", framestyle = :box, fg_legend = :transparent, ylim=(-10,12))
	...
	# filename = "Fig-AMD-Profit-Short-Strangle.pdf"
	# savefig(joinpath(_PATH_TO_FIGS, filename))
end

# ╔═╡ b6532480-f407-4845-a77f-49be6130e457
md"""
#### Case II: Long Strangle
"""

# ╔═╡ c4a1a608-6ef8-483f-b744-85371ce1cbf4
let
	...
	call_contract.premium = 𝒫₂
	call_contract.number_of_contracts = 1
	call_contract.direction = 1 # LONG
	...
	# plot -
	plot(dt[!, :S], dt[!,:profit],c=:black, label="total profit", legend=:top, lw=3, 
		bg="floralwhite", background_color_outside="white", framestyle = :box, fg_legend = :transparent, ylim=(-12,12))
	...
	# uncomment me to save fig to a file -
	# filename = "Fig-AMD-Profit-Long-Strangle.pdf"
	# savefig(joinpath(_PATH_TO_FIGS, filename))
end

# ╔═╡ b44f3fc6-36ec-4d5e-a85c-3811b088f28f
md"""
...
# ╔═╡ 34a5eede-14d4-11ed-3ad6-41414e31a600
html"""
<style>
main {
    max-width: 1200px;
