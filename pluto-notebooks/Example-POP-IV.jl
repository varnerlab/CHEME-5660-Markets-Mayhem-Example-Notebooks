...
# ╔═╡ cb024ee5-65b8-4a29-9cff-6f9c60c9c009
begin
...
# ╔═╡ aa86a388-7e6a-456c-a3e6-82254879b151
md"""
### CHEME 5660: Computing the Probability of Profit (POP) using the Implied Volatilty
"""

# ╔═╡ 240f5dda-cf48-49d1-957c-2cd4a3903435
md"""
#### Introduction
"""

# ╔═╡ e36d8c01-8f60-433d-802a-a34fd0d1ba17
md"""
#### Materials and Methods
"""

# ╔═╡ 67823ab5-6038-409a-9ca7-addb28008c44
# load the AMD price data -
df = CSV.read(joinpath(_PATH_TO_DATA, "AMD-OHLC-from-2020-10-18-to-2022-10-18.csv"), DataFrame);

# ╔═╡ 5c49aa7c-1c9d-46c4-b1b2-e5f5a55dec4b
...
# ╔═╡ 8a6e773c-7aea-445e-8ca6-a424e0f874b8
...
# ╔═╡ 4a13160c-2ef5-4c63-af39-c9fda6c4e6ff
...
# ╔═╡ 9dc489d1-3ce4-4c32-a7ed-8e68b7583fd0
md"""
#### Results
"""

# ╔═╡ 2abea08f-a34d-4a9e-abb3-9a0a535d0157
begin
...
# ╔═╡ 5ac0bb8d-cec4-4f1d-ab3e-44aaf5f296da
md"""
##### a) Estimate the $\mu$ parameter from the training data
Suppose there was no noise in the market; then the volatility parameter $\sigma=0$. In this case, the Geometric Brownian Motion (GBM) solution becomes deterministic:

$$X(t) = X_{\circ}e^{{\mu}\left(t-t_{\circ}\right)}$$
...
__Strategy__:
Let $A$ denote the $\mathcal{S}\times{2}$ matrix holding the time values; the first column of $A$ is all 1's while the second column holds the $(t-t_{\circ})$ values. Further, let $Y$ denote the AMD close price values (in the same order as the $A$ matrix). Then, the y-intercept and slope can be estimated by solving the `overdetermined` system of equations:
...
# ╔═╡ 8ee3c260-1d53-45e7-aeab-774c9ee5e2fa
begin
...
# ╔═╡ f67f9e9e-1ac1-4ff6-b381-fc3c60625124
# compute θ -
θ = inv(transpose(A)*A)*transpose(A)*Y

# ╔═╡ 0d60796f-c8f1-4b29-924b-badd0b720adb
Ŷ = A*θ
...
md"""
##### b) Estimate the Cumulative Distribution Function (CDF) for share price
"""

# ╔═╡ 837a5174-1d74-4b3f-ab51-4965e9a2c0e2
begin

	ΔT = 58.0
	μ̂ = θ[2]
	Var_market = ((Sₒ)*(IV/100.0)*sqrt(ΔT/365))^2
	T_market = (ΔT/365.0)
	...
	training_model.T₂ = (ΔT/365.0)
	training_model.h = (1.0/365.0)
	training_model.Xₒ = Sₒ
	training_model.μ = μ̂;
	training_model.σ = σ̂;
...
begin
	d = fit_mle(LogNormal,sample_simulation[end,2:end])
	#d = LogNormal(3.85,0.45)
	u_price_array = range(1,step=0.01,stop=100.0) |> collect
	number_of_price_steps = length(u_price_array)
	tmp_array = Array{Float64,2}(undef, number_of_price_steps, 2)
	for i ∈ 1:number_of_price_steps
		S = u_price_array[i]
		tmp_array[i,1] = S
		tmp_array[i,2] = cdf(d, S)
	end
end
...
# ╔═╡ 12a84d88-1a37-45ca-8d9b-30ba43c6f4db
begin
	μₘ = log(Sₒ*exp(μ̂*(ΔT/365.0)))
	σ̂^2
end

# ╔═╡ dd53294b-454b-46c4-9d73-3499e0149d70
md"""
...
# ╔═╡ 6955884c-4fe2-11ed-3f90-7513eed687d8
html"""
<style>
main {
    max-width: 1200px;
