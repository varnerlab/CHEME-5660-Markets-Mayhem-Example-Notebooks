...
# ╔═╡ 9a43608e-94a7-48e3-9c3e-0a0f1c940de5
md"""
##### Plot the single index model versus data
"""

# ╔═╡ 3704500d-b3b9-48ed-8272-3346ae99a2c8
begin

	# how many days of data do we have?
	(NTR,_) = size(price_df_spy)

	# compute the return array -
	r_spy_all = compute_return_array(𝒫_spy; m = (NTR-1));
	r_amd_all = compute_return_array(𝒫_amd; m = (NTR-1));
	
	# formulate a X matrix will all the data -
	XALL = Array{Float64,2}(undef, NTR-1, 2)
	for i ∈ 1:(NTR-1)
		XALL[i,1] = 1.0
		XALL[i,2] = r_spy_all[i] - rf;
	end
...
# ╔═╡ e97d2def-57ab-4816-83d3-1f4c38353eac
md"""
__Figure__: Excess daily return for SPY versus the excess daily return for AMD. Points denote data. Line denotes the prediction of the Single Index Model (SIM).
"""
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "596c0556bcb4833e79c47e2d1fc2760a5a7c336a"
