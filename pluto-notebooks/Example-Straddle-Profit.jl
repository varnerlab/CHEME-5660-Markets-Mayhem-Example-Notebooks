...
md"""
### CHEME 5660: Calculating the Profit of a Straddle at Expiration
"""
...
let
	...
	push!(straddle_contract_array, short_put_contract)
	push!(straddle_contract_array, short_call_contract)
	
	# setup the underlying -
	L = 1000
	underlying_range = range(80.0, stop = 120.0, length = L) |> collect
...
let
	...
	long_call_contract.premium = 𝒫₂
	long_call_contract.number_of_contracts = 1
	long_call_contract.direction = 1

	# build model -
	straddle_contract_array = Array{AbstractAssetModel,1}()
	push!(straddle_contract_array, long_put_contract)
	push!(straddle_contract_array, long_call_contract)
