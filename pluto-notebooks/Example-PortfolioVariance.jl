...
On the other hand, the variance (risk squared) of the portfolio is defined as:
...
# Fill me in
# ----------------------------------------------------------------------------------------------- #
function compute_binary_portfolio_rerturn(i::Int64, j::Int64, w₁::Float64, μ::Array{Float64,1})::Float64

	# initialize -
	rₚ = 0.0 	# default: zero portfolio 
	w₂ = 1 - w₁ # allocation of w₂ 

	# grab the individual returns -
	μ₁ = μ[i]
	μ₂ = μ[j]
...
begin
	...
	# compute the binary portfolio return -
	rarray = Float64[]
	for j ∈ 1:𝒫
		preturn = compute_binary_portfolio_rerturn(1,j,w,μᵣ)
		push!(rarray,preturn)
	end
end
...
with_terminal() do
	...
	for i ∈ 1:𝒫
		...
		data_table[i,2] = (1/μᵣ[1])*rarray[i]
		data_table[i,3] = sqrt(varray[i])/sqrt(varray[1])
...
begin
	# plot (exclude META) -
	scatter(sqrt.(varray[1:end .!= 12])*(1/sqrt(varray[1])), rarray[1:end .!=12]*(1/rarray[1]), 
		label="XYZ (w=$(1-w)) + APPL (w=$(w))", legend=:topleft, c=:black)
	xlabel!("Scaled Portfolio Risk (scaled to AAPL)",fontsize=18)
...
* The closing price data for this example was downloaded from [Polygon.io](https://polygon.io) using the [PQPolygonSDK.jl package](https://github.com/Paliquant/PQPolygonSDK.jl)
"""
