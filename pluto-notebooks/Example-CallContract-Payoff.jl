...
# ╔═╡ 734ebe7a-03c7-4778-aeed-479ea2bf2878
begin
...
# ╔═╡ faed8e0a-c850-4049-9a4b-4fb02e99cbd2
md"""
### CHEME 5660: Calculating the Payoff of Call and Put Contracts at Expiration
"""

# ╔═╡ d2297ebf-d1af-4008-a85f-d359b464853e
html"""
...
Suppose an investor purchases a call contract for the underlying stock `XYZ` for $\mathcal{P}$ USD/contract, where the contract controls 100 shares of `XYZ.` 
...
# ╔═╡ 8b890d56-92bc-437e-9fd6-0f974b7e5989
abstract type AbstractContract end 

# ╔═╡ 28d64689-67d0-49d2-8c9c-0bf04658abec
mutable struct Put <: AbstractContract
...
mutable struct Call <: AbstractContract
...
function payoff(contract::T, S::Array{Float64,1}) where T <: AbstractContract

	# get data from contract -
	K = contract.K
	
	# initialize -
	payoff_array = Array{Float64,1}()

	# main loop -
	for Sᵢ ∈ S

		P = 0.0
		if (isa(contract, Call) == true)
			P = max(Sᵢ - K,0)
		elseif (isa(contract, Put) == true)
			P = max(K - Sᵢ, 0)
		end
		push!(payoff_array, P)
	end
