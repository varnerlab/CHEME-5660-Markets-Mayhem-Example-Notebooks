...
# ╔═╡ 270d982d-a8ad-4816-88cd-021af29bbbcb
begin
	# load packages -
	using Plots
	using Distributions
...
# ╔═╡ d8528cf4-4272-11ed-10e0-91ca2b2ae7c4
md"""
### CHEME 5660: Geometric Brownian Motion (GBM) Model Calibation
"""

# ╔═╡ 3c8d01fe-5127-40ec-9620-9801e85d7b93
html"""
...
# ╔═╡ 234dcd32-b258-49c7-aa64-ad559ba9c60c
md"""
#### Introduction
"""

# ╔═╡ f3ef13a9-2ad2-421d-ac79-8c6ab328b284
md"""
Fill me in.
"""

# ╔═╡ ead05ff1-17a6-4875-8f34-61d351ebd557
md"""
#### Materials and Methods
"""

# ╔═╡ d02740e1-45e3-49fc-91d2-438d7f8a46d3
function variance(data::DataFrame, key::Symbol)::Array{Float64,1}

	# initialize -
	(NR,NC) = size(data)
	variance_vector = zeros(NR)

	for i ∈ 1:NR
		idx_range = range(1,stop=i,step=1)
		tmp_array = data[idx_range,key]
		var_value = var(tmp_array; corrected=false)
		variance_vector[i] = var_value
	end
...
# ╔═╡ 2d0b6f57-d5a4-444b-9e44-dddfda27c5d7
function gbm_expectation_variance(μ::Float64, σ::Float64; 
	Xₒ::Float64 = 1.0, T::Float64 = 14.0, h::Float64 = 1.0)::Array{Float64,2}

	# output array -
	time_array = range(0,stop=T,step=h) |> collect
	number_of_time_steps = length(time_array)
	output_array = Array{Float64,2}(undef, number_of_time_steps, 3)
	...
	for i ∈ 1:number_of_time_steps
		...
		# data -
		t = time_array[i]
		expectation = Xₒ*exp(μ*t)
		variance = (Xₒ^2)*exp(2*μ*t)*(exp((σ^2)*t) - 1)

		# capture -
		output_array[i,1] = t
		output_array[i,2] = expectation
		output_array[i,3] = variance
	end
...
# ╔═╡ 6f91c418-2bc5-4461-abe0-b6adebfdfb15
function solve(μ::Float64, σ::Float64; 
	Xₒ::Float64 = 1.0, T::Float64 = 14.0, 𝒫::Int64 = 100, 
	h::Float64 = 1.0)::Tuple{Array{Float64,1}, Array{Float64,2}}

	# initialize -
	time_array = range(0,stop=T,step=h) |> collect
	number_of_time_steps = length(time_array)
	soln_array = zeros(number_of_time_steps, 𝒫)
	...
	for p ∈ 1:𝒫
		for t ∈ 1:number_of_time_steps-1
			soln_array[t+1,p] = soln_array[t,p]*exp((μ - σ^2/2)*h + σ*(sqrt(h))*ZM[t,p])
		end
	end

	# return -
	return (time_array, soln_array)
end
...
begin
	df_all = CSV.read(joinpath(_PATH_TO_DATA, "QQQ-OHLC-day-2017-08-25-to-2022-09-23.csv"),
		DataFrame);
...
begin
	...
	(NR,NC) = size(df)
	PA = log.(df[!,:close])
	tmp = range(1,stop=NR,step=1) |> collect
	T = tmp*(1/365.0) .- (1/365.0)
end

# ╔═╡ 14c25a44-3d1e-40bf-86b1-2a87c6c249d7
begin
	Y = Vector(PA[1:ℒ])
	XD = [ones(length(T[1:ℒ])) T[1:ℒ]]
	θ = inv(transpose(XD)*XD)*transpose(XD)*Y
end
...
# ╔═╡ 0e707aeb-89a8-4333-a73b-3aef5688c73c
θ

# ╔═╡ 5a958625-acb4-4995-8a8d-334fb0c8c8dc
begin
	σ̂ = 0.12
	vm = gbm_expectation_variance(θ[2], σ̂; T = (ℒ/365.0), Xₒ=Sₒ, h=(1.0/356.0))
	vv = variance(df,Symbol("close"))
	plot(T,vv)
	plot!(vm[:,1], vm[:,3],lw=2)
end

# ╔═╡ 26fbc465-9c33-4beb-a4f5-e137a5093e7a
begin	
	...
	# solve -
	(TA,XA) = solve(μ̂,σ̂; T = (1279/365.0), Xₒ = Xₒ , h=(1.0/356.0));

	# compute the mean var -
	X_mean_var = gbm_expectation_variance(μ̂, σ̂; T = (1279/365.0), Xₒ = Xₒ, h=(1.0/356.0))

	# show -
	nothing
end

# ╔═╡ 2ee5067d-2b53-4d8c-8676-4eb40560f2bf
begin
	
	U = X_mean_var[:,2] .+ 1*sqrt.(X_mean_var[:,3])
	L = X_mean_var[:,2] .- 1*sqrt.(X_mean_var[:,3])
	
	plot(TA,XA,label="", c=colorant"#BDBBBB")
	plot!(X_mean_var[:,1], X_mean_var[:,2],c=:black,lw=2)
	plot!(X_mean_var[:,1], X_mean_var[:,2],c=colorant"#0068AC",
		fillrange=U, fillalpha = 0.4, lw=2, label="μ ± σ")
	plot!(X_mean_var[:,1], X_mean_var[:,2],c=colorant"#0068AC",
		fillrange=L, fillalpha = 0.4, lw=2, label="")

	tmp_all = range(1,stop=1279,step=1) |> collect
	T_all = tmp_all*(1/365.0) .- (1/365.0)
	plot!(T_all, df_all[!,:close],lw=2, c=:green)
	plot!(T, df[!,:close], lw=3, c=:red)
end

# ╔═╡ c55a1192-df11-4a0e-95da-982d69d8d982
md"""
...
# ╔═╡ 8144db43-b082-47d2-a79c-56fda239e5dc
html"""
<style>
main {
    max-width: 1200px;
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "551479ed87626aef774917aedaab0140f454af89"
