...
# ╔═╡ ece6472c-52d6-11ed-2c7a-b19d778282f2
begin
...
md"""
The [Jade-Lizard](https://www.tastytrade.com/concepts-strategies/jade-lizard) is a composite options trade consisting of three option legs:
* We sell 1 $\times$ OTM (out of the money) Put contract with strike price $K_{1}$ USD/share
* We sell 1 $\times$ OTM Call contract with strike price $K_{2}$ USD/share
* We buy 1 $\times$ OTM Call contract with strike price $K_{3}>K_{2}$ USD/share.

The [Jade-Lizard](https://www.tastytrade.com/concepts-strategies/jade-lizard) was created by [tasty-traders Liz and JNY](https://www.tastytrade.com/shows/the-liz-jny-show?locale=en-US).
"""

# ╔═╡ 4c8b0fcc-72b1-43e4-b840-74b82c0a65e7
...
# ╔═╡ a64010d3-ccb7-4666-a083-e98c4b5c7796
begin
	...
	# build model -
	jade_lizard_contract_array = Array{AbstractAssetModel,1}()
	push!(jade_lizard_contract_array, short_put_contract_model)
	push!(jade_lizard_contract_array, short_call_contract_model)
	push!(jade_lizard_contract_array, long_call_contract_model)
	
	# setup the underlying -
	underlying_range = range(45.0, stop = 80.0, length = 1000) |> collect
	
	# compute the table -
	dt = expiration(jade_lizard_contract_array, underlying_range)

	# show -
	nothing
end

# ╔═╡ 6ab01af9-0490-4b0a-a9ac-bd28e763793d
begin

	plot(dt[!, :S], dt[!,:profit],c=:black, label="total profit", legend=:bottomleft, lw=3, bg="floralwhite", 	background_color_outside="white", framestyle = :box, fg_legend = :transparent)

	plot!(dt[!,:S], dt[!, "profit_$(T₁)"], c=colorant"#EF4035", label="short put K=$(K₁)", lw=1, ls=:dash)
	plot!(dt[!,:S], dt[!, "profit_$(T₂)"], c=colorant"#6EB43F", label="short call K=$(K₂)", ls=:dash, lw=1)
	plot!(dt[!,:S], dt[!, "profit_$(T₃)"], c=colorant"#0068AC", label="long call K=$(K₃)", lw=1, ls=:dash)

	# plot break-even line -
	ZL = zeros(1000)
	plot!(dt[!,:S], ZL, c=colorant"#9FAD9F", label="Breakeven", lw=1, ls=:dash)	

	# add labels -
	xlabel!("Underlying share price at expiration (USD/share)", fontsize=18)
	ylabel!("Profit (USD/share)", fontsize=18)
end

# ╔═╡ 22231321-41c8-4d76-8984-b9c798cd834f
html"""
<style>
main {
    max-width: 1200px;
