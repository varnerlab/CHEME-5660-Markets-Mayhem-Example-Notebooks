...
# ╔═╡ eac2d9cb-5f06-401c-a217-4ddda7f5cee3
# ------------------------------------------------------------------------------------------- #
# Fill me in with some awesome stuff
# ------------------------------------------------------------------------------------------- #
...
# ╔═╡ 829bc347-222c-4d9d-be96-ec52f0c61fca
# ------------------------------------------------------------------------------------------- #
# Fill me in with some awesome stuff
# ------------------------------------------------------------------------------------------- #
function loss(κ, V̂ᵦ::Float64, Vₚ::Float64, T::Int64, c̄::Float64)::Float64

	# unpack the unknown parameter vector -
	r̄ = κ[1] # guess for the market interest rate 
...
# ╔═╡ b0ea113a-a2c1-49f7-8e5f-69d12e60ce55
with_terminal() do
	
	bgfs_soln = Optim.minimizer(opt_result)[1]
	println("Yield to Maturity (YTM) annual rate r̄ = $(bgfs_soln*100)%")
end

# ╔═╡ c976f0aa-4c79-4511-9f64-27c90d6ca4f1
md"""
b) __Check__: can we recover the sale price if we plug in the YTM value for r̄ in the `price` function?
"""

# ╔═╡ 24a27483-f1a4-4b3e-a06f-2fbc0937c694
begin

	# get the value of the optimal solution from the Optim result object -
	r̄_check =  Optim.minimizer(opt_result)[1]

	# compute the price?
	V̂ᵦ_check = price(Vₚ, T, c̄, r̄_check)

	# display -
	with_terminal() do
		println("Check: V̂ᵦ_check = $(V̂ᵦ_check) USD")
	end
end
...
* This example was inspired by Example 14.4 of BKM: Zvi Bodie, Alex Kane, and Alan J Marcus. Investments. The McGraw-Hill/Irwin series in finance, insurance, and real estate. McGraw-Hill/Irwin, New York, 9th ed edition, 2011. ISBN 9780073530703 (alk. paper).
"""
