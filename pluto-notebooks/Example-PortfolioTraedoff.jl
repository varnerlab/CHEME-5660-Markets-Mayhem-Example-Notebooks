...
# ╔═╡ e8be6e16-c82b-4ca1-912a-0e38c2a5c75c
begin
...
# ╔═╡ fb582f08-9c80-44a5-b53e-a344531cc134
md"""
## CHEME 5660: The Risk Versus Reward Tradeoff for a Multicomponent Portfolio
Jeffrey D. Varner, Smith School of Chemical and Biomolecular Engineering, Cornell University, Ithaca, NY 14850
"""

# ╔═╡ 0dfff72b-1073-420e-9977-ec3c9cc828fd
md"""
### Introduction
Fill me in.
"""

# ╔═╡ 92b963a0-a31c-4590-b0f7-6b3243476520
md"""
### Materials and Methods
"""

# ╔═╡ b56a311b-ead6-41f0-88cf-cedcac8c6e55
# -------------------------------------------------------------------------------------------------------------- #
# Fill me in
# -------------------------------------------------------------------------------------------------------------- #
...
# ╔═╡ 16715d49-9f29-4a94-afe1-36fdc95d0183
# -------------------------------------------------------------------------------------------------------------- #
# Fill me in
# -------------------------------------------------------------------------------------------------------------- #
...
We generate a random collection of possible portfolio weight vectors such that each vector obeys the summation constraint:
...
begin
	...
	# initialize -
	N = 10000 # how many portfolios do we want to look at?
	Nₚ = length(data_dictionary)
	W = Array{Float64,2}(undef, Nₚ, N)

	# populate -
	for j ∈ 1:N
		w = generate_asset_allocation(Nₚ)
		for i ∈ 1:Nₚ
			W[i,j] = w[i]
		end
	end
...
begin
	...
	RR = Array{Float64,2}(undef, N, 2)
	for i ∈ 1:N

		# get the potential allocation -
		w = W[:,i]
		
		# compute the Risk -
		risk = sqrt(transpose(w)*Σ*w)
		reward = transpose(w)*μᵣ
...
git-tree-sha1 = "ae908790a904a4534a76ad41e7d1c898732202db"
