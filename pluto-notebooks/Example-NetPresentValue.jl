...
md"""
## CHEME 5660: Net Present Value Calculations and Decision Making
...
### Introduction
The discrete Net Present Value (NPV) is the sum of the current and future cash flows computed over the time horizon $t=1\rightarrow{T}$, corrected to current dollars ($t=1$):

$$\text{NPV} = \sum_{t=1}^{T}{\mathcal{D}_{t,1}^{-1}}\cdot{CF_{t}}$$

The terms $CF_{t}$ denotes the cash flow in time period $t$, while $\mathcal{D}_{t,1}$ denotes the discrete multistep discount factor between the current time period ($t=1$), and time period $t$. We know that $r_{1,1} = 0$ because there is no possibility of an instantaneous return; thus, $\mathcal{D}_{t,1} = 1$, and the NPV can be re-written as:
...
The term $\dot{C}_{s}$ denotes money stream $s$, while $\nu_{s}$ denotes the stream direction parameter; if a money stream enters the time node $\star$ (revenue) then $\nu_{s}=+1$, on the other hand if a money stream exits time node $\star$ (expense) then $\nu_{s}=-1$. 

##### What is the discount rate?
The discount rate $r_{t+1,t}$ will be company-specific as it's related to how the company gets its funds. It's the rate of return that the investors expect or the cost of borrowing money. If shareholders expect a 12% return, that is the discount rate the company will use to calculate NPV. If the firm pays 4% interest on its debt, it may use that figure as the discount rate. Typically the chief financial officer sets the rate.
"""
